...
betas <- rnorm(10,0,2)
#Set some betas to 0
betas[sample(1:length(betas),5)] <- 0
...
alfa <- 5
mu <- -10
pi <- 0.5   #Needs to be thought about
...
for(ind in 1:length(BETA)){
  new_beta <- ars(1,function(w) f_beta_vec(w,alfa,mu,ind,gamma_vec),function(w) f_beta_derivative_vec(w,alfa,mu,ind,gamma_vec),x=abscissae_set$beta[,ind],m=5)
  BETA[ind] <- new_beta
}
...
for(i in 1:n){
  ...
  for(ind in 1:length(BETA)){
    ...
    #Full likelihood is required (not just the proportionality)
    l_spike <- log(1-pi) + sum(data_set$failure==1)*log(alfa) + sum(data_set$failure*(mu + X[,-ind] %*% BETA[-ind]) + data_set$failure *(alfa-1)*data_set$time_in_research - 
                                                                      exp(mu + X[,-ind] %*% BETA[-ind])*data_set$time_in_research)  
    
    l_slab  <- log(pi) + sum(data_set$failure==1)*log(alfa) + sum(data_set$failure*(mu + X %*% BETA) + data_set$failure *(alfa-1)*data_set$time_in_research - 
                                                                   exp(mu + X %*% BETA)*data_set$time_in_research)+
      0.5*sum(gamma_vec)*log(1/sigma2) - (1/(2*sigma2) * sum(BETA*BETA) )
    
    gamma_vec[ind] <- (l_spike/(l_spike+l_slab))>runif(1)  # 
  }
  ...
  sigma2 <- rinvgamma(1,shape=sigma_alfa+0.5*sum(gamma_vec),scale=1/(0.5* sum((BETA[gamma_vec==1])**2)+sigma_beta))  

  theta <- rnorm(50,0,1)
  sigma_test <- rinvgamma(1,shape=sigma_alfa+0.5*sum(gamma_vec),scale=1/(0.5* sum((theta[gamma_vec==1])**2)+sigma_beta))  
...
sum(res_matrix[500:1000,12]==0)

hist(res_matrix[500:1000,1])
